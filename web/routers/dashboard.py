...
def dashboard_summary(
    ...
    """返回当前用户的 Dashboard 概览数据."""
    today_alert_count = db.get_today_alert_count(user["id"])
    latest_price_count = db.get_latest_price_records_count()  # 市场公共数据
    ...
    try:
        ...
        _, by_day = db.get_alert_stats(
            user_id=user["id"], start_date=yesterday_str, end_date=yesterday_str
        )
    ...
    return DashboardSummary(
        active_watchlist=db.get_watchlist_count(user["id"], enabled_only=True),
        extreme_track_count=db.get_extreme_track_count(user["id"], enabled_only=True),
